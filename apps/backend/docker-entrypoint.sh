...
export CORTEX_RUNTIME="${CORTEX_RUNTIME:-container}"
export CORTEX_HOME="${CORTEX_HOME:-/app}"
...
case "${1:-}" in
  ...
  upgrade-check)
    ...
    ;;
  *)
    echo "[entrypoint] Runtime: $CORTEX_RUNTIME, Home: $CORTEX_HOME"
    ...
    if [ $upgrade_result -eq 2 ]; then
      ...
      if [ "${CORTEX_LOCAL_BUILD:-}" = "true" ]; then
        echo "  Run: docker compose -f compose.yaml -f compose.local.yaml run --rm cortex upgrade"
      else
        echo "  Run: docker compose run --rm cortex upgrade"
      fi
      echo "================================================"
      echo ""
      # Sleep forever to prevent restart loops - container will show as running but unhealthy
