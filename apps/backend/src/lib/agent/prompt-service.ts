...
import type { AIMessage, ToolCallSummaryOutput } from "@cortex/ai";
import {
...
import { graphitiClient, type GraphitiFact } from "../graphiti/index.js";
...
/**
 * Format knowledge graph facts into a context string for the system prompt
 */
function formatGraphitiFacts(facts: GraphitiFact[]): string | undefined {
  if (facts.length === 0) return undefined;
  return facts
    .map((f) => {
      const dateInfo = f.valid_at ? ` (as of ${f.valid_at.slice(0, 10)})` : "";
      return `- ${f.fact}${dateInfo}`;
    })
    .join("\n");
}

/**
 * Ingest a conversation exchange into the knowledge graph (fire-and-forget)
 */
function ingestConversationToKG(
  userId: string,
  prompt: string,
  responseText: string,
): void {
  // Fire-and-forget - catch any errors to prevent them from propagating
  graphitiClient
    .addMessages(userId, [
      {
        role_type: "user",
        content: prompt,
        source_description: "cortex conversation",
      },
      {
        role_type: "assistant",
    ...
    .catch(() => {
      // Intentionally swallowed - logging happens in graphitiClient
    });
}
  ...
  return new ToolLoopAgent<BackendAgentContext>({
    ...
    instructions: (context) => {
      return buildSystemPrompt({
        ...
        graphitiContext: options.graphitiContext,
      });
    },
...
export async function processPromptRequest(
  ...
  try {
    ...
    // Search knowledge graph for relevant facts (non-blocking)
    const kgFacts = await graphitiClient.search(prompt, userId, 5);
    const graphitiContext = formatGraphitiFacts(kgFacts);

    // Create agent with KG context
    ...
    // Ingest conversation into knowledge graph (fire-and-forget)
    ingestConversationToKG(userId, prompt, result.text);
