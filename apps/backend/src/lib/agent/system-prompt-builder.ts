...
import { toOpenAITools } from "@cortex/ai";
...
export interface BuildSystemPromptOptions {
  ...
  /** Relevant facts from knowledge graph memory */
...
export function buildSystemPrompt(options: BuildSystemPromptOptions): string {
  ...
  // Build relevant memory section from knowledge graph
  let relevantMemorySection = "";
  if (graphitiContext) {
    relevantMemorySection = `

## Relevant Memory

You have access to relevant facts from previous conversations and knowledge:
${graphitiContext}

Use this context to provide more personalized and informed responses when relevant.`;
  }
...
Current Date & Time: ${currentDateString} (${currentTimeString})${userContextInfo}${relevantMemorySection}${assetContentSection}`;
