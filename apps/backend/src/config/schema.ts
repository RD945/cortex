...
export type CortexRuntime = "local" | "container";
...
export function buildConfig(): CortexConfig {
  ...
  // Runtime context
  const runtime = (env.CORTEX_RUNTIME || "local") as CortexRuntime;
  const isContainer = runtime === "container";
  const isProduction = env.NODE_ENV === "production";

  // Home directory - anchor for all paths
  const home = env.CORTEX_HOME || (isContainer ? "/app" : ".");
  ...
  // Set DATABASE_HOST in process.env so @cortex/db uses correct host
  ...
  const dbUser = env.DATABASE_USER || "cortex";
  const dbPassword = env.DATABASE_PASSWORD || "cortex";
  const dbName = env.DATABASE_NAME || "cortex";
  ...
  const config: CortexConfig = {
    ...
    queue: {
      ...
      redisKeyPrefix: env.REDIS_KEY_PREFIX || "cortex",
    },
    ...
    // Graphiti Knowledge Graph
    graphiti: {
      enabled: env.GRAPHITI_ENABLED === "true",
      url:
        env.GRAPHITI_URL ||
        (isContainer ? "http://graphiti:8000" : "http://127.0.0.1:8765"),
    },
 ...
 */
export function validateConfig(config: CortexConfig): string[] {
  const errors: string[] = [];
