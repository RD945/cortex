...
export function getAppVersion(): string {
  ...
  if (process.env.CORTEX_RUNTIME === "container") {
 ...
 *   - node_modules/@cortex/db/src/migrations (bundled by pnpm deploy)
 *   - /app/migrations (explicit copy in Dockerfile)
 *
 * Local dev (monorepo):
 *   - node_modules/@cortex/db/dist/migrations (if packages are built)
...
function getMigrationPaths(): string[] {
  ...
  return [
    // Container: pnpm deploy bundles @cortex/db with src/
    resolve(backendRoot, "node_modules/@cortex/db/src/migrations"),
    // Container: explicit copy in Dockerfile to /app/migrations
    resolve(backendRoot, "migrations"),
    // Local dev with built packages
    resolve(backendRoot, "node_modules/@cortex/db/dist/migrations"),
...
/**
 * Database dialect types (matches @cortex/db DbDialect)
 */
