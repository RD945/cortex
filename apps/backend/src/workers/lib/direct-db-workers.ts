 ...
 * Uses createDbWorker from @cortex/queue/driver-db with event callbacks
 * for real-time SSE updates.
 */

import { runWithRequestId } from "@cortex/logger";
import { createEventCallbacks, QueueNames } from "@cortex/queue/app";
import type {
  JobContext,
  JobEventCallbacks,
  Worker,
} from "@cortex/queue/core";
import {
  createDbWorker,
  type DbWorkerConfig,
  getQueueSchema,
  type DbCapabilities as QueueDbCapabilities,
} from "@cortex/queue/driver-db";
...
import {
  processGraphitiIngest,
  type GraphitiIngestJobData,
} from "../jobs/graphitiIngestProcessor.js";
...
function getCapabilities(): QueueDbCapabilities {
  // Map from @cortex/db capabilities to @cortex/queue capabilities
...
export async function startDirectDbWorkers(): Promise<void> {
  ...
  // Graphiti ingest worker
  const graphitiIngestWorker = createDbWorker(
    QueueNames.GRAPHITI_INGEST,
    async (ctx: JobContext<GraphitiIngestJobData>) => {
      await processGraphitiIngest(ctx);
    },
    config,
    { concurrency: 1 },
  );
  workers.push(graphitiIngestWorker);
  logger.info(
    { queue: QueueNames.GRAPHITI_INGEST },
    "Graphiti ingest worker started",
  );
