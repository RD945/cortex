...
import { isAIInitialized, validateAIConfigOnStartup } from "@cortex/ai";
import { runWithRequestId } from "@cortex/logger";
import { QueueNames } from "@cortex/queue/app";
import type { Worker } from "@cortex/queue/core";
import {
  type BullMQWorkerConfig,
  createBullMQWorker,
} from "@cortex/queue/driver-bullmq";
...
import processGraphitiIngest from "./jobs/graphitiIngestProcessor.js";
import {
...
export async function startBullMQWorkers(): Promise<void> {
  ...
  // Graphiti Ingest Worker (long-running bulk job)
  const graphitiIngestWorker = createBullMQWorker(
    QueueNames.GRAPHITI_INGEST,
    processGraphitiIngest,
    workerConfig,
    longTaskOptions,
  );
  bullmqWorkers.push(graphitiIngestWorker);
