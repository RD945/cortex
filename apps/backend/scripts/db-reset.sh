...
case "$DB_TYPE" in
    postgres|postgresql)
        ...
        if ! docker ps --format "table {{.Names}}" | grep -q "^cortex-postgres$"; then
            error "PostgreSQL container 'cortex-postgres' is not running. Start it with: docker compose -f compose.yaml -f compose.dev.yaml up -d postgres"
        fi

        # Drop and recreate database
        info "Dropping and recreating database 'cortex'..."
        docker exec cortex-postgres psql -U cortex -d postgres -c "DROP DATABASE IF EXISTS cortex;" 2>/dev/null || true
        docker exec cortex-postgres psql -U cortex -d postgres -c "CREATE DATABASE cortex;"
