...
export function MainLayoutClient({ children }: MainLayoutClientProps) {
  ...
  const getDeviceInfo = () => ({
    ...
    app: { name: "Cortex Frontend", version: packageJson.version },
  });
      ...
      <div className="flex flex-1 overflow-hidden main-layout-container">
                  ...
                  {(item as Record<string, unknown>).isDialog ? (
                    <FeedbackDialog
                      trigger={
                        <button
                          type="button"
                          className="flex items-center gap-3 px-3 py-2 rounded-md text-[15px] font-medium text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-[hsl(var(--hover-bg))] w-full text-left transition-colors"
                        >
                    ...
                    <Link
                      to={item.href}
                      className={`flex items-center gap-3 px-3 py-2 rounded-md text-[15px] transition-colors ${
                        isActive(item.href)
                          ? "font-semibold"
                          : "font-medium text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-[hsl(var(--hover-bg))]"
                      }`}
                      style={
                        isActive(item.href)
                          ? {
        ...
        {assistantOpen && !assistantFullScreen && (
          ...
          >
            ...
            <div
              className={`absolute left-0 top-0 bottom-0 w-2 cursor-col-resize hover:bg-foreground/20 ${
