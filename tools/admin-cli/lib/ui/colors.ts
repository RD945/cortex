...
export function formatEngine(
  engine: { name: string; managed?: boolean } | undefined,
): string {
...
export function formatContext(contexts: string[] | undefined): string {
  ...
  return contexts
    .map((ctx: string) => {
      switch (ctx) {
        case "backend":
          return chalk.blue(ctx);
        case "workers":
          return chalk.green(ctx);
        default:
          return chalk.white(ctx);
      }
    })
    .join(colors.dim(", "));
}
...
export function formatAuthType(authType: string): string {
  switch (authType) {
    ...
    case "bearer":
      return chalk.green("bearer");
    case "api-key-header":
      return chalk.blue("api-key");
    default:
      return chalk.white(authType);
  }
}
...
export function formatDialect(dialect: string): string {
  switch (dialect) {
    case "openai-chat":
      return chalk.blue("openai-chat");
    case "mlx-responses":
      return chalk.yellow("mlx-responses");
    default:
      return chalk.white(dialect);
  }
}
...
export function printProviderReminder(
  ...
  console.log(
    colors.warning(
      `- Make sure to download the model for ${colors.emphasis(provider)}`,
    ),
  );

  if (provider === "llama-cpp") {
    console.log(colors.info("- Start the engine with: cortex engine up"));
    console.log(
  ...
  console.log(
    colors.dim("https://github.com/eclaire-labs/eclaire#selecting-models"),
  );
}
