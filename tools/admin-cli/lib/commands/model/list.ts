...
export async function listCommand(options: CommandOptions): Promise<void> {
  try {
    ...
    console.log(
      colors.dim(
        "  cortex model activate [ID]   - Activate a model (interactive if no ID)",
      ),
    );
    console.log(
      colors.dim(
        "  cortex model deactivate [ctx] - Deactivate model for context",
      ),
    );
    console.log(
      colors.dim("  cortex model info <ID>       - Show detailed model info"),
    );
    console.log(
      colors.dim("  cortex model import <url>    - Import new model"),
    );
    console.log(colors.dim("  cortex model remove <ID>     - Remove a model"));
  } catch (error: unknown) {
