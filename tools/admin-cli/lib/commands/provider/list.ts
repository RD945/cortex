...
export async function listCommand(options: CommandOptions): Promise<void> {
  try {
    ...
    if (providerIds.length === 0) {
      ...
      console.log(colors.dim('\nRun "cortex provider add" to add a provider'));
      return;
    }
    ...
    console.log(colors.dim("\nCommands:"));
    console.log(
      colors.dim("  cortex provider add          - Add a new provider"),
    );
    console.log(
      colors.dim("  cortex provider edit <id>    - Edit a provider"),
    );
    console.log(
      colors.dim(
        "  cortex provider test <id>    - Test provider connectivity",
      ),
    );
    console.log(
      colors.dim("  cortex provider remove <id>  - Remove a provider"),
    );
  } catch (error: unknown) {
