...
export async function addCommand(options: CommandOptions): Promise<void> {
  try {
    ...
    if (preset.id === "custom") {
        ...
        const { managed } = await inquirer.prompt([
          {
            type: "confirm",
            name: "managed",
            message: "Should Cortex manage this server (start/stop via CLI)?",
      ...
      if (preset.defaultEngine) {
        ...
        if (canBeManaged) {
          const { managed } = await inquirer.prompt([
            {
              ...
              name: "managed",
              message:
                "Should Cortex manage this server (start/stop via CLI)?",
              default: true,
            },
          ]);
          ...
          console.log(
            colors.dim(
              `Engine '${preset.defaultEngine.name}' is external - Cortex will connect but not start/stop it`,
            ),
          );
        }
      }
    ...
    if (!preset.isCloud) {
      ...
      console.log(
        colors.dim(`Test connectivity: cortex provider test ${providerId}`),
      );
    } else {
      console.log(
        colors.dim(`\nTest connectivity: cortex provider test ${providerId}`),
      );
    }
  } catch (error: unknown) {
