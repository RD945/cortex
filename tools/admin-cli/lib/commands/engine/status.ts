...
export async function statusCommand(options: CommandOptions): Promise<void> {
  try {
    ...
    if (!engineStatus.running && resolution.status === "ok") {
      console.log(colors.dim("Start engine: cortex engine up"));
    } else if (engineStatus.running) {
      console.log(colors.dim("Stop engine:  cortex engine down"));
      console.log(colors.dim("View logs:    cortex engine logs"));
    }
