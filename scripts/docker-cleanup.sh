...
clean_unused_networks() {
    ...
    # Don't remove the cortex-net network as it's actively used
...
# Function to clean old Cortex images (keep current + previous for rollback)
clean_old_cortex_images() {
    print_info "Cleaning old Cortex images..."
    ...
    # Find and remove old/duplicate Cortex images
    ...
    print_success "Old Cortex images cleaned (kept current + previous versions)"
}
...
show_help() {
    echo "Cortex Docker Cleanup Utility"
    echo ""
    echo "Usage: $0 [OPTION]"
    echo ""
    echo "Options:"
    ...
    echo "  --old-images        Remove old Cortex images only"
