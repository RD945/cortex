...
import type { DbInstance, TransactionManager } from "@cortex/db";
import type { Logger } from "@cortex/logger";
...
export interface GraphitiIngestJobData extends JobData {
  userId: string;
  /** Which sources to ingest (all true by default) */
...
export interface QueueAdapter {
  ...
  enqueueGraphitiIngest(data: GraphitiIngestJobData): Promise<void>;
  close(): Promise<void>;
}
...
export interface RedisQueueConfig {
  ...
  url: string;
  /** Redis key prefix for BullMQ (default: "cortex") */
  prefix?: string;
}
