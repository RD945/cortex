/**
 * @cortex/queue/app - Application-specific queue types and adapters
 *
 * This subpath contains Cortex-specific types and adapters that are
 ...
 * - @cortex/queue (main entry)
 * - @cortex/queue/core (zero-dependency types)
 * - @cortex/queue/driver-db (database driver)
 * - @cortex/queue/driver-bullmq (Redis/BullMQ driver)
 */
 ...
 * ```typescript
 * // Redis mode
 * import { createQueueAdapter } from "@cortex/queue/app";
 * import { createLogger } from "@cortex/logger";
 *
 ...
 * ```typescript
 * // Database mode
 * import { createQueueAdapter, createJobWaitlist } from "@cortex/queue/app";
 * import { initializeDatabase } from "@cortex/db";
 * import { createLogger } from "@cortex/logger";
 *
 * const logger = createLogger({ service: "backend" });
...
export async function createQueueAdapter(
  ...
  if (mode === "redis") {
    ...
    logger.info(
      { prefix: config.redis.prefix || "cortex" },
