...
export function createBullMQAdapter(config: BullMQAdapterConfig): QueueAdapter {
  ...
  return {
    ...
    async enqueueGraphitiIngest(data: GraphitiIngestJobData): Promise<void> {
      const queue = queueManager.getQueue(QueueNames.GRAPHITI_INGEST);
      if (!queue) {
        logger.error({}, "Failed to get Graphiti ingest queue");
        throw new Error("Queue not available");
      }
      ...
      await queue.add(
        ...
        {
          // Use userId as jobId to prevent duplicate ingest jobs per user
          jobId: `graphiti-ingest:${data.userId}`,
        },
      );

      logger.info(
        { userId: data.userId },
        "Graphiti ingest job enqueued to Redis",
      );
    },
