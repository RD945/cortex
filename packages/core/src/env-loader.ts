// Shared environment loader for Cortex
//
...
//
// In containers (CORTEX_RUNTIME=container):
...
//
// In local development (CORTEX_RUNTIME=local or unset):
...
if (!isContainer && !isTest && !loaded) {
  ...
  process.env.CORTEX_RUNTIME ??= "local";
  process.env.CORTEX_HOME ??= rootDir;
}
