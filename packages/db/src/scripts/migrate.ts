/**
 * Database migration script for @cortex/db.
 *
 * Supports PostgreSQL, PGlite, and SQLite databases.
 * Usage:
 *   pnpm --filter @cortex/db db:migrate
 *   pnpm --filter @cortex/db db:migrate --status
 *   pnpm --filter @cortex/db db:migrate --force
 */

// Load environment FIRST - uses shared loader from @cortex/core
// Importing @cortex/core triggers env loading as a side effect
import "@cortex/core";
