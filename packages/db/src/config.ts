...
export function getDatabaseUrl(): string | null {
  ...
  const isContainer = process.env.CORTEX_RUNTIME === "container";
  ...
  const database = process.env.DATABASE_NAME || "cortex";
  const username = process.env.DATABASE_USER || "cortex";
  const password = process.env.DATABASE_PASSWORD || "cortex";
...
export function getPGlitePath(): string {
  ...
  // Use CORTEX_HOME if set (container mode), otherwise use __dirname relative path (dev mode)
